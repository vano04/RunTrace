        ...
        <div className="overflow-hidden rounded-xl border bg-card shadow-sm lg:grid lg:grid-cols-[0.8fr_1.2fr]">
          <section className="border-b bg-muted/25 p-7 lg:border-r lg:border-b-0 lg:p-10">
            ...
            <ol className="mt-10 space-y-7">
              {[
                [Users, isInvite ? t("Identity created") : t("Create owner identity"), isInvite ? t("An admin has granted your access.") : t("You’ll own and administer this instance.")],
                [KeyRound, t("Choose a password"), t("Use at least 12 characters and keep it in a password manager.")],
                [ShieldCheck, t("Ready for your team"), t("Each person receives a separate identity and password.")],
          ...
          <section className="p-7 lg:p-10">
            ...
            <form className="mt-7 space-y-5" onSubmit={(event) => { event.preventDefault(); void submit() }}>
              ...
              <div className="space-y-2"><Label htmlFor="password">{t("Password")}</Label><Input id="password" type="password" autoComplete={isBootstrap || isInvite ? "new-password" : "current-password"} minLength={isBootstrap || isInvite ? 12 : undefined} value={password} onChange={(event) => setPassword(event.target.value)} required /></div>
