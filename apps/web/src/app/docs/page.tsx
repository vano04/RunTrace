...
export default function DocsPage() {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-7xl px-5 sm:px-8">
    <header className="flex h-20 items-center justify-between border-b"><RunTraceLogo /><Button variant="ghost" render={<Link href="/" />} nativeButton={false}><ArrowLeft data-icon="inline-start" />{t("Projects")}</Button></header>
...
with run(
    ...
    tracked.finish("kept", "3.24 at step 1000", "Improved the baseline reproducibly.")`}</Code>
          <p>{rich("If a run already exists, start the process with {command}. The SDK validates and attaches to that running record instead of creating a duplicate.", { command: <code className="rounded bg-muted px-1.5 py-0.5">RUNTRACE_RUN_ID=run_... python train.py</code> })}</p>
        </DocSection>
...
finish_run({ run_id: "run_...", disposition: "kept", result_summary: "...", conclusion: "..." })`}</Code>
        ...
        <DocSection id="plugins" title={t("Codex and Claude Code")} icon={Plug}>
          <p>{rich("Install the PyPI tool and repository marketplace from either CLI. The plugin bundles the RunTrace skill, starts the authenticated MCP server with {command}, and rereads the CLI's saved credential on every request.", { command: <code className="rounded bg-muted px-1.5 py-0.5">uvx</code> })}</p>
